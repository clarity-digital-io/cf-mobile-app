...
import { ResponseSchema, FormSchema, QuestionSchema, AnswerSchema, QuestionOptionSchema, QuestionCriteriaSchema, sObjectSchema } from '../schema';
...
const onAuthRealm = async (user) => {
	...
	try {
		...
		const config = { 	sync: { user: user, url: `${REALM_URL}/~/user`, fullSynchronization: true, validate_ssl: false },  schema: [ResponseSchema, sObjectSchema] };
...
const onGlobalAuthRealm = async (user, organization_id) => {
	...
	try {
		...
		const config = { 	sync: { user: user, url: `${REALM_URL}/${organization_id}/forms`, fullSynchronization: true, validate_ssl: false },  schema: [FormSchema, QuestionSchema, QuestionOptionSchema, QuestionCriteriaSchema] };
